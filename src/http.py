...
class HttpServer:
    ...
    def _init_game_state(self):
        ...
        self.game_state = {
            ...
            'round_completed_start_time': None, 'round_completed_duration': 2.0,
    ...
    def check_disconnected_players(self):
        now = time.time()
        gs = self.game_state
        disconnected = [p for p in gs['connected_players']
                        if now - gs['last_heartbeat'].get(p, now) > gs['heartbeat_timeout']]
    ...
    def reset_game_internal(self):
        players = self.game_state['connected_players'].copy()
        now = time.time()
        self.game_state.update({
            ...
            'timesup_state': False, 'timesup_start_time': None, 'timesup_duration': 3,
            'round_completed_state': False, 'round_completed_start_time': None,
            'round_completed_duration': 2.0, 'advancing_question': False,
        })
    ...
    def start_countdown(self):
        gs = self.game_state
        gs['countdown_started'] = True
        gs['countdown_start_time'] = time.time()
        print(f"🔻 Starting countdown with {len(gs['connected_players'])} players")

    def start_game(self):
        gs = self.game_state
        gs.update({
            'game_started': True, 'countdown_started': False,
            'game_start_time': time.time(), 'current_question_number': 1,
    ...
    def get_game_status(self, player_id):
        with self.question_lock:
            ...
            should_advance, advance_reason = False, None
            if elapsed >= gs['question_duration']:
                if not all_answered:
                    ...
                    if rem <= 0:
                        should_advance, advance_reason = True, "timesup_finished"
                        gs['timesup_state'] = False
                        gs['timesup_start_time'] = None
                    else:
                        if player_has_answered:
                            return {'status': 'roundcompleted_waiting', 'timesup_remaining': rem,
                                    'current_question_number': gs['current_question_number'],
                                    'max_questions': gs['max_questions'], 'game_started': True, 'player_answered': True,
                                    'answered_players': list(gs['answered_players']),
                                    'waiting_for_players': list(gs['connected_players'] - gs['answered_players'])}
                        else:
                            return {'status': 'timesup', 'timesup_remaining': rem,
                                    'current_question_number': gs['current_question_number'],
                                    'max_questions': gs['max_questions'], 'game_started': True, 'player_answered': False,
                                    'answered_players': list(gs['answered_players']),
                                    'waiting_for_players': list(gs['connected_players'] - gs['answered_players'])}
                else:
                    should_advance, advance_reason = True, "time_and_all_answered"
            elif all_answered:
                ...
                if rem <= 0:
                    should_advance, advance_reason = True, "all_answered_completed"
                    gs['round_completed_state'] = False
                    gs['round_completed_start_time'] = None
                else:
                    return {'status': 'roundcompleted_all', 'roundcompleted_remaining': rem,
                            'current_question_number': gs['current_question_number'],
                            'max_questions': gs['max_questions'], 'game_started': True, 'all_answered': True,
                            'answered_players': list(gs['answered_players']),
                            'total_players': len(gs['connected_players'])}
            if should_advance and not gs.get('advancing_question', False):
                return self._advance_question_safely(now, advance_reason)
            return {'status': 'playing', 'game_started': True,
                    'current_question_number': gs['current_question_number'],
                    'max_questions': gs['max_questions'],
                    'question_time_remaining': max(0, gs['question_duration'] - elapsed),
    ...
    def _advance_question_safely(self, now, reason):
        ...
        try:
            ...
            gs['current_question'] = self.generate_new_question()
    ...
    def post_answer(self, data):
        with self.question_lock:
            ...
            if is_correct:
                ...
                return {'status': 'correct', 'correct': True, 'new_score': gs['player_scores'][player_id],
            ...
            return {'status': 'incorrect', 'correct': False, 'new_score': gs['player_scores'].get(player_id, 0),
